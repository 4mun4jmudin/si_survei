<x-admin-layout>
    <x-slot name="title">Tambah Data Siswa</x-slot>
    ...
    <div class="max-w-3xl mx-auto">
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 sm:p-8">
            <div class="mb-6">
                ...
                <p class="text-sm text-gray-500">Isi detail data siswa responden survei.</p>
            </div>
            ...
            <form action="{{ route('admin.siswa.store') }}" method="POST" class="space-y-6">
                ...
                <div>
                    ...
                    @enderror
                    <p class="mt-1.5 text-xs text-gray-500">Hanya menampilkan akun dengan role 'siswa' yang belum ditautkan.</p>
                </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="nis" id="nis" value="{{ old('nis') }}" required class="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 focus:bg-white transition @error('nis') border-red-500 @enderror" placeholder="Contoh: 10293847">
                        @error('nis')
                            <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                        @enderror
                    </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="kelas" id="kelas" value="{{ old('kelas') }}" required class="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 focus:bg-white transition @error('kelas') border-red-500 @enderror" placeholder="Contoh: X-IPA-1, XI-IPS-2">
                        @error('kelas')
                            <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                        @enderror
                    </div>
