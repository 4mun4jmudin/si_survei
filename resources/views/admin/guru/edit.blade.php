<x-admin-layout>
    ...
    <x-slot name="header">Edit Guru: {{ $guru->nama_lengkap }}</x-slot>

    <div class="max-w-3xl mx-auto">
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 sm:p-8">
            <div class="mb-6">
                <h3 class="text-lg font-semibold text-gray-800">Informasi Guru</h3>
                <p class="text-sm text-gray-500">Perbarui detail data guru atau staf pengajar.</p>
            </div>
            ...
            <form action="{{ route('admin.guru.update', $guru) }}" method="POST" class="space-y-6">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="nip" id="nip" value="{{ old('nip', $guru->nip) }}" required class="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 focus:bg-white transition @error('nip') border-red-500 @enderror" placeholder="Contoh: 198001012005011001">
                        @error('nip')
                            <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" name="nama_lengkap" id="nama_lengkap" value="{{ old('nama_lengkap', $guru->nama_lengkap) }}" required class="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 focus:bg-white transition @error('nama_lengkap') border-red-500 @enderror" placeholder="Contoh: Budi Santoso, S.Pd.">
                        @error('nama_lengkap')
                            <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                        @enderror
                    </div>
