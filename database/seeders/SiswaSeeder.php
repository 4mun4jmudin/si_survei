...
class SiswaSeeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        $dummyUsers = \App\Models\User::where('name', 'like', 'Siswa %')->get();
        foreach ($dummyUsers as $user) {
            \App\Models\Siswa::where('user_id', $user->id)->delete();
            $user->delete();
        }
        ...
        $data = [
            ...
            'XII RPL 2' => [
                ['nis' => '232410001B', 'nama' => 'ARNI NURAINI'],
        ...
        foreach ($data as $kelas => $siswas) {
            foreach ($siswas as $s) {
                $username = $s['nis'];
                $cleanNis = str_replace('B', '', $username);

                // Skip if already exists
                if (\App\Models\User::where('username', $username)->exists()) continue;
