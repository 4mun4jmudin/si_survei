...
class PertanyaanSeeder extends Seeder
{
    ...
    {
        $kuesioner = \App\Models\Kuesioner::where('nama_kuesioner', 'like', '%Lingkungan Belajar%')->first();
        if (!$kuesioner) return;
        ...
        foreach ($pg_questions as $q) {
            if (\App\Models\Pertanyaan::where('isi_pertanyaan', $q['isi'])->exists()) continue;
            
            $order = \App\Models\Pertanyaan::where('kuesioner_id', $kuesioner_id)->max('nomor_urutan') + 1;
