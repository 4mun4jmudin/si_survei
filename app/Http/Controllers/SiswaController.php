...
class SiswaController extends Controller
{
    public function index()
    {
        ...
        // Ambil kuesioner yang aktif dan belum diisi oleh siswa ini
        ...
        return view('siswa.dashboard', compact('kuesionerTersedia', 'kuesionerSelesai'));
    }
    ...
    public function showDetail(Kuesioner $kuesioner)
    {
        ...
        if ($sudahIsi) return redirect()->route('siswa.dashboard')->with('error', 'Anda sudah mengisi kuesioner ini.');

        $pertanyaanCount = $kuesioner->pertanyaan()->count();
        return view('siswa.survei_detail', compact('kuesioner', 'pertanyaanCount'));
    }
    ...
    public function showSurvei(Kuesioner $kuesioner)
    {
        ...
        if ($sudahIsi) {
            return redirect()->route('siswa.dashboard')->with('error', 'Anda sudah mengisi kuesioner ini.');
        }

        if ($kuesioner->status !== 'aktif') {
            return redirect()->route('siswa.dashboard')->with('error', 'Kuesioner ini sudah tidak aktif.');
        }
    ...
    public function simpanSurvei(Request $request, Kuesioner $kuesioner)
    {
        ...
        try {
            ...
            return redirect()->route('siswa.dashboard')->with('success', 'Terima kasih telah mengisi survei! Kontribusi Anda sangat berharga.');
        } catch (\Exception $e) {
            DB::rollBack();
